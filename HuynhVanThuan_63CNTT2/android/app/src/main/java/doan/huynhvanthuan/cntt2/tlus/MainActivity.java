...
import doan.huynhvanthuan.cntt2.tlus.model.Fruit;
...
public class MainActivity extends AppCompatActivity {
    ...
    EditText txtsearch;
    Button btnThem, btnXoa, btnSua,btnLuu,btnlamlai;
    EditText txtId, txtName, txtSoluong, txtGia, txtNgaymua;
    int flag = 0;
    SQLiteDatabase db;
    Cursor cs;
...
//      Hiển thị
    ...
    void initView() {
        ...
        btnlamlai.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                resetView(); // Reset lại các trường nhập liệu
                showDataListView(); // Cập nhật lại danh sách hiển thị
                flag = 0; // Đặt lại cờ
                btnThem.setEnabled(true);
                btnSua.setEnabled(true);
                Toast.makeText(MainActivity.this, "Đã làm lại", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        btnLuu.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (flag == 1) {
                    ...
                    String sql = "INSERT INTO " + DBHelperDatabaseFruit.TABLE_FRUITS + " (name, quantity, price, purchaseDate) " +
                            "VALUES ('" + name + "', '" + quantity + "', '" + price + "', '" + purchaseDate + "')";
                    try {
                        db.execSQL(sql);
                        dbHelper.close();
                        showDataListView();
